...
const columns: Array<{ key: SortKey; label: string }> = [
  ...
  { key: "startTarget", label: "Start Target" },
...
function normalizeTrade(value: unknown): PaperTrade | null {
  ...
  return {
    source: row.source === "PAPER" ? "PAPER" : "BACKTEST",
...
export default function PaperLedger() {
  ...
  const displayed = useMemo(() => {
    ...
    return base;
  }, [rows, year, month, callType, pnlFilter, sortKey, sortDir]);
  ...
  return <section className={styles.ledger} id="trade-ledger">
    <div className={styles.heading}>
      <div><p className={styles.eyebrow}>Backtest + live paper journal</p><h2>Trade ledger</h2>
        <p>Historical rows use the momentum-trailing model. New paper trades append automatically.</p></div>
    ...
    <div className={styles.tableWrap}><table><thead><tr>{columns.map((column) => <th key={column.key}>
      ...
      {displayed.map(({ trade, originalRow }, displayIndex) => <tr key={`${trade.date}-${trade.callType}-${trade.strikePrice}-${originalRow}`}>
        <td>{displayIndex + 1}</td><td>{trade.date}</td><td>{trade.indexStockName}</td><td>{trade.weeklyExpiry}</td>
        <td>{trade.lots}</td><td><span className={`${styles.optionBadge} ${trade.callType === "CE" ? styles.ce : styles.pe}`}>{trade.callType}</span></td>
        <td>{num.format(trade.strikePrice)}</td><td>₹{num.format(trade.startTarget)}</td><td>₹{num.format(trade.startStopLoss)}</td><td>₹{num.format(trade.endStopLoss)}</td>
        <td>{trade.entryTime || "—"}</td><td>{trade.exitTime || "—"}</td><td>{trade.stopLossAdjustments}</td>
        <td className={trade.totalPnl >= 0 ? styles.profit : styles.loss}>{trade.totalPnl >= 0 ? "+" : "−"}₹{money.format(Math.abs(trade.totalPnl))}</td>
      </tr>)}
      {!displayed.length && <tr><td className={styles.empty} colSpan={14}>No trades match the selected filters.</td></tr>}
    </tbody></table></div>
    <p className={styles.footnote}>Paper mode only. P/L is simulated and includes the configured transaction-cost model; no broker order is placed.</p>
  </section>;
}
