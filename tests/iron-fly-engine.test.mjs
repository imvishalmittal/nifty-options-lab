...
import {
  IRON_FLY_RULES,
  attachIronFlyCosts,
  evaluateIronFlyPosition,
  netIronFlyCredit,
  selectIronFlyContracts,
  summarizeIronFlyResults,
} from '../research/opportunity/iron-fly-engine.mjs';
...
test('selects one shared nearest-ATM short strike with exact symmetric wings', () => {
  const contracts = [];
  for (let strike = 23500; strike <= 24500; strike += 50) {
    contracts.push(contract(strike, 'CE'), contract(strike, 'PE'));
  }
  const selected = selectIronFlyContracts(contracts, {
    spot: 24000,
    range: { high: 24100, low: 23900, width: 200 },
  });
  assert.equal(selected.shortCall.strike, 24000);
  assert.equal(selected.shortPut.strike, 24000);
  assert.equal(selected.longCall.strike, 24200);
  assert.equal(selected.longPut.strike, 23800);
});

test('requires an exact equal-width protective wing', () => {
  const contracts = [
    contract(24000, 'CE'), contract(24000, 'PE'),
    contract(24200, 'CE'), contract(23850, 'PE'),
  ];
  assert.equal(selectIronFlyContracts(contracts, {
    spot: 24000,
    range: { high: 24100, low: 23900, width: 200 },
  }), null);
});
...
test('uses completed close for threshold detection and exits all legs at next-bar open', () => {
  ...
  const position = evaluateIronFlyPosition({
    legCandles: legCandlesByName,
    entryTimestamp: '2026-08-24T10:00:00+05:30',
  });
  assert.equal(netIronFlyCredit(position.entryQuotes), 60);
...
test('marks a session missing instead of inventing one of four threshold quotes', () => {
  ...
  const result = evaluateIronFlyPosition({
    legCandles: legs,
    entryTimestamp: '2026-08-24T10:00:00+05:30',
  });
...
test('eight-order costs and slippage are applied across all four legs', () => {
  ...
  const clean = attachIronFlyCosts(position, { lotSize: 65, tradeDate: '2026-08-24' });
  const stressed = attachIronFlyCosts(position, { lotSize: 65, tradeDate: '2026-08-24', slippagePointsPerLeg: 1 });
...
test('summary keeps the iron-fly research stream independent', () => {
  const results = [
  ...
  const summary = summarizeIronFlyResults(results);
  ...
  assert.equal(IRON_FLY_RULES.entryTime, '10:00');
});
