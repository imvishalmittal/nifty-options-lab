...
import {
  simulateCashProgram,
  simulateFixedTicketProgram,
  xirr,
} from '../research/etf-dip-recovery-cash-program.mjs';
...
test('fixed ticket uses recycled proceeds before requesting fresh funding', () => {
  const sessions = ['2025-01-02', '2025-01-03', '2025-01-06', '2025-01-07'];
  const trades = [
    {
      date: '2025-01-02', symbol: 'AAA', targetReturnPct: 10, status: 'TARGET',
  ...
  const result = simulateFixedTicketProgram({
    sessions,
    trades,
    startDate: '2025-01-02',
    fundingMonths: 1,
  ...
  assert.equal(result.totalPurchased, 30_000);
  assert.equal(result.totalFreshFunding, 15_000);
  assert.equal(result.recycledCashUsed, 15_000);
  assert.equal(result.terminalValue, 18_000);
  assert.equal(result.profit, 3_000);
  assert.equal(result.purchaseLedger[1].freshFunding, 0);
});
