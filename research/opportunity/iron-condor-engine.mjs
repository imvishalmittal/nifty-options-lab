...
export const IRON_CONDOR_STRATEGY = 'intraday-range-iron-condor';

export const IRON_CONDOR_RULES = Object.freeze({
  openingRangeStart: '09:15',
  openingRangeEnd: '10:00',
  entryTime: '10:00',
  forcedExit: '15:10',
  maximumAdx: 20,
  maximumOpeningRangePct: 0.006,
  maximumEmaSpreadPct: 0.0015,
  shortDistancePct: 0.01,
  openingRangeBufferPoints: 50,
  wingWidthPoints: 200,
  minimumCreditPoints: 5,
  profitCaptureRatio: 0.5,
  stopDebitMultiple: 2,
});
...
export function selectIronCondorContracts(contracts, { spot, range, rules = IRON_CONDOR_RULES }) {
  ...
  const callTarget = Math.max(range.high + rules.openingRangeBufferPoints, spot * (1 + rules.shortDistancePct));
  const putTarget = Math.min(range.low - rules.openingRangeBufferPoints, spot * (1 - rules.shortDistancePct));
  const shortCall = parsed.filter((row) => row.optionType === 'CE' && row.strike >= callTarget)
    .sort((a, b) => a.strike - b.strike)[0] ?? null;
  const shortPut = parsed.filter((row) => row.optionType === 'PE' && row.strike <= putTarget)
  ...
  return {
    ...
    targets: { callTarget, putTarget },
  };
}

export function detectIronCondorRegime(candles, rules = IRON_CONDOR_RULES) {
  ...
  return { status: 'SIGNAL', strategy: IRON_CONDOR_STRATEGY, evidence };
}

export function detectIronCondorSetup(candles, contracts, rules = IRON_CONDOR_RULES) {
  const regime = detectIronCondorRegime(candles, rules);
  if (regime.status !== 'SIGNAL') return regime;
  const selection = selectIronCondorContracts(contracts, {
...
export function netCondorCredit(prices) {
...
function exitFromQuotes({ entryQuotes, exitQuotes, entryTime, exitTime, result, rules, thresholdTime = null }) {
  const entryCredit = netCondorCredit(entryQuotes);
  const exitDebit = netCondorCredit(exitQuotes);
  return {
    status: 'TRADE',
    entryTime,
    exitTime,
...
export function evaluateIronCondorPosition({ legCandles, entryTimestamp, rules = IRON_CONDOR_RULES }) {
  ...
  for (const timestamp of timestamps) {
    ...
    const debit = netCondorCredit(closeQuotes);
...
export function attachIronCondorCosts(position, { lotSize, tradeDate, slippagePointsPerLeg = 0 }) {
...
export function summarizeIronCondorResults(results) {
