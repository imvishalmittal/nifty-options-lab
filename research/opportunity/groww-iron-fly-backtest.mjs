...
export function normalizeIronFlyCandles(raw = []) {
...
async function fetchCandles(token, { segment, symbol, startTime, endTime }) {
  ...
  return normalizeIronFlyCandles(payload.candles ?? []);
}
...
function attachCostScenarios(position, lotSize, tradeDate) {
  ...
  return {
    ...
    costs: {
      normalized: attachIronFlyCosts(position, { lotSize, tradeDate }),
      stress0_5: attachIronFlyCosts(position, { lotSize, tradeDate, slippagePointsPerLeg: 0.5 }),
      stress1_0: attachIronFlyCosts(position, { lotSize, tradeDate, slippagePointsPerLeg: 1 }),
    },
  };
}

export async function backtestIronFly({
  token,
  startDate,
  endDate,
  lotSize,
  spacing = DEFAULT_REQUEST_SPACING_MS,
  rules = IRON_FLY_RULES,
}) {
  ...
  for (const [date, candles] of [...sessions.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    const signal = detectIronFlyRegime(candles, rules);
    ...
    const selection = selectIronFlyContracts(contractsByExpiry.get(expiry), {
    ...
    const position = attachCostScenarios(evaluateIronFlyPosition({ legCandles, entryTimestamp, rules }), appliedLotSize, date);
    results.push({
      date,
...
async function main() {
  ...
  const output = await backtestIronFly({
    token,
...
if (process.argv[1]?.endsWith('groww-iron-fly-backtest.mjs')) {
  main().catch((error) => {
