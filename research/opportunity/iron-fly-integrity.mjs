...
import { IRON_FLY_STRATEGY } from './iron-fly-engine.mjs';
...
export function validateIronFlyResult(document) {
  const errors = [];
  const warnings = [];
  if (document?.schemaVersion !== 1) errors.push('schemaVersion must be 1');
  if (document?.strategy !== IRON_FLY_STRATEGY) errors.push(`strategy must be ${IRON_FLY_STRATEGY}`);
...
if (process.argv[1]?.endsWith('iron-fly-integrity.mjs')) {
