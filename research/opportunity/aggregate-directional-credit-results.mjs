...
import { DIRECTIONAL_CREDIT_STRATEGY, summarizeDirectionalCreditResults } from './directional-credit-engine.mjs';
import { validateDirectionalCreditResult } from './directional-credit-integrity.mjs';
...
export function aggregateDirectionalCreditDocuments(documents) {
...
function main() {
  ...
  const consolidated = aggregateDirectionalCreditDocuments(files.map((file) => JSON.parse(fs.readFileSync(file, 'utf8'))));
...
if (process.argv[1]?.endsWith('aggregate-directional-credit-results.mjs')) {
