...
import { summarizeOpportunityResults } from './opportunity-engine.mjs';
...
export function aggregateDocuments(documents, expectedStrategy) {
  if (!documents.length) throw new Error('No partition result documents found');
  for (const document of documents) {
    if (document.strategy !== expectedStrategy) throw new Error(`Expected ${expectedStrategy}, found ${document.strategy}`);
  ...
  return {
    ...
    diagnostics: {
      ...
      vwapFallbackSessions: ordered.reduce((sum, document) => sum + (document.diagnostics?.vwapFallbackSessions ?? 0), 0),
    },
...
function main() {
  ...
  if (!args.dir || !args.strategy || !args.out) throw new Error('--dir, --strategy and --out are required');
  ...
  const consolidated = aggregateDocuments(documents, args.strategy);
...
if (process.argv[1]?.endsWith('aggregate-results.mjs')) {
