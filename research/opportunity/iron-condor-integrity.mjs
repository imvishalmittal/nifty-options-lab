...
import { IRON_CONDOR_STRATEGY } from './iron-condor-engine.mjs';
...
export function validateIronCondorResult(document) {
  const errors = [];
  const warnings = [];
  if (document?.schemaVersion !== 1) errors.push('schemaVersion must be 1');
  if (document?.strategy !== IRON_CONDOR_STRATEGY) errors.push(`strategy must be ${IRON_CONDOR_STRATEGY}`);
...
if (process.argv[1]?.endsWith('iron-condor-integrity.mjs')) {
