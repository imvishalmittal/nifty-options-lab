...
import { IRON_CONDOR_STRATEGY, summarizeIronCondorResults } from './iron-condor-engine.mjs';
import { validateIronCondorResult } from './iron-condor-integrity.mjs';
...
export function aggregateIronCondorDocuments(documents) {
  if (!documents.length) throw new Error('No partition result documents found');
  for (const document of documents) {
    const report = validateIronCondorResult(document);
...
function main() {
  ...
  const consolidated = aggregateIronCondorDocuments(files.map((file) => JSON.parse(fs.readFileSync(file, 'utf8'))));
...
if (process.argv[1]?.endsWith('aggregate-iron-condor-results.mjs')) {
