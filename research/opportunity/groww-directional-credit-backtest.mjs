...
import {
  DIRECTIONAL_CREDIT_RULES,
  DIRECTIONAL_CREDIT_STRATEGY,
...
} from './directional-credit-engine.mjs';
import {
...
export function normalizeDirectionalCreditCandles(raw = []) {
...
async function fetchCandles(token, { segment, symbol, startTime, endTime }) {
  ...
  return normalizeDirectionalCreditCandles(payload.candles ?? []);
}
...
function attachCostScenarios(position, lotSize, tradeDate) {
  ...
  return {
    ...
    costs: {
      normalized: attachDirectionalCreditCosts(position, { lotSize, tradeDate }),
      stress0_5: attachDirectionalCreditCosts(position, { lotSize, tradeDate, slippagePointsPerLeg: 0.5 }),
      stress1_0: attachDirectionalCreditCosts(position, { lotSize, tradeDate, slippagePointsPerLeg: 1 }),
    },
  };
}

export async function backtestDirectionalCredit({
  token,
  startDate,
  endDate,
  lotSize,
  spacing = DEFAULT_REQUEST_SPACING_MS,
  rules = DIRECTIONAL_CREDIT_RULES,
}) {
  ...
  for (const [date, candles] of [...sessions.entries()].sort(([a], [b]) => a.localeCompare(b))) {
    ...
    const signal = detectDirectionalCreditRegime(candles, rules);
    ...
    const selection = selectDirectionalCreditContracts(contractsByExpiry.get(expiry), {
      spot: signal.evidence.spot,
      direction: signal.direction,
      rules,
    });
    if (!selection) {
      results.push({ date, status: 'DATA_MISSING', reason: 'Required exact-width two-leg credit spread unavailable', signal, expiry });
      continue;
    }
    const legCandles = {};
    ...
    const position = attachCostScenarios(evaluateDirectionalCreditPosition({ legCandles, entryTimestamp, rules }), appliedLotSize, date);
    results.push({
      date,
      strategy: DIRECTIONAL_CREDIT_STRATEGY,
      signal,
      expiry,
      selection,
      ...position,
  ...
  return {
    ...
    executionModel: {
      ...
      option: 'nearest weekly NIFTY expiry strictly after the session; 0.5%-OTM directional short with exact 200-point protective wing',
      ...
      costs: 'four Groww option orders plus statutory charges; 0/0.5/1.0 adverse premium-point slippage on every leg entry and exit',
    ...
    summary: summarizeDirectionalCreditResults(results),
    results,
  };
}
...
async function main() {
  ...
  const output = await backtestDirectionalCredit({
    token,
...
if (process.argv[1]?.endsWith('groww-directional-credit-backtest.mjs')) {
  main().catch((error) => {
