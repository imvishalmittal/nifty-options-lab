...
import { nearestExpiry } from './nifty-180-premium-strategy.mjs';
...
async function expiries(token,underlying,years,spacingMs){const out=[];for(const year of years){const p=await get(token,'/historical/expiries',{exchange:'NSE',underlying_symbol:underlying,year},spacingMs);out.push(...(p.expiries??[]));}return [...new Set(out)].sort();}
async function contracts(token,underlying,expiry,spacingMs){const p=await get(token,'/historical/contracts',{exchange:'NSE',underlying_symbol:underlying,expiry_date:expiry},spacingMs);return (p.contracts??[]).map((row)=>parseIndexOptionContract(row,underlying)).filter(Boolean);}
...
export async function backtestProfitOnly({token,startDate,endDate,lotSize='auto',spacingMs=1500,signals=SIGNALS,exits=EXITS,tradeBothSides=false,underlying='NIFTY',spotSymbol='NSE-NIFTY'}){
  lastRequestAt=0;const lookback=addDays(startDate,-120);const spot=await candles(token,{segment:'CASH',symbol:spotSymbol,start:lookback,end:endDate},spacingMs);const groups=groupSpotSessions(spot);const daily=dailySessions(groups);const dates=[...groups.keys()].filter(d=>d>=startDate&&d<=endDate).sort();const expiryList=await expiries(token,underlying,[...new Set(dates.map(d=>Number(d.slice(0,4))))],spacingMs);const contractCache=new Map(),optionCache=new Map();const keys=signals.flatMap(s=>exits.map(e=>`${s}_${e}`));const variants=Object.fromEntries(keys.map(k=>[k,[]]));const sessions=[];
  for(const date of dates){const rows=groups.get(date);const dayIndex=daily.findIndex(d=>d.date===date);const previous=daily[dayIndex-1]??null;const priorCloses=daily.slice(0,dayIndex).map(d=>d.close);const candidateSignals=[openingRangeSignal(rows),retestSignal(rows),previousDayBreakSignal(rows,previous),regimeAlignedSignal(rows,priorCloses),vwapContinuationSignal(rows)].filter(Boolean);if(!candidateSignals.length){sessions.push({date,status:'NO_TRADE'});continue;}const expiry=nearestExpiry(expiryList,date);if(!expiry){sessions.push({date,status:'DATA_MISSING',reason:'No expiry'});continue;}if(!contractCache.has(expiry))contractCache.set(expiry,await contracts(token,underlying,expiry,spacingMs));const chain=contractCache.get(expiry);const spot925=rows.find(r=>r.timestamp.slice(11,16)==='09:25')?.open;if(!Number.isFinite(spot925)){sessions.push({date,status:'DATA_MISSING',reason:'No 09:25 spot'});continue;}
    for(const signal of candidateSignals.filter((row)=>signals.includes(row.strategy))){const directions=tradeBothSides?['UP','DOWN']:[signal.direction];for(const direction of directions){const contract=selectItmContract(chain,spot925,direction);if(!contract)continue;const cacheKey=`${date}:${contract.symbol}`;if(!optionCache.has(cacheKey))optionCache.set(cacheKey,await candles(token,{segment:'FNO',symbol:contract.symbol,start:date,end:date},spacingMs));const optionRows=optionCache.get(cacheKey);const signalOptionRow=optionRowAt(optionRows,signal.signal.timestamp);if(!signalOptionRow)continue;const datedLotSize=lotSize==='auto'?indexLotSizeForExpiry(underlying,expiry):Number(lotSize);if(!(datedLotSize>0))continue;for(const exit of exits){const position=evaluateImmediateBreakeven(optionRows,signal.signal.timestamp,exit);if(!position)continue;const key=`${signal.strategy}_${exit}`;const side=direction==='UP'?'CE':'PE';const legCapital=tradeBothSides?CAPITAL/2:CAPITAL;variants[key].push({date,strategy:signal.strategy,exitMode:exit,side,underlying,contract,expiry,lotSize:datedLotSize,signalOpenInterest:signalOptionRow.openInterest,signalVolume:signalOptionRow.volume,spot925,signalDirection:signal.direction,signalTime:signal.signal.timestamp,allocatedCapital:legCapital,...position,money:{current:costs(position,date,datedLotSize,'current',legCapital),stress0_5:costs(position,date,datedLotSize,'stress0_5',legCapital),stress1_0:costs(position,date,datedLotSize,'stress1_0',legCapital)}});}}}
    sessions.push({date,status:'PROCESSED',signals:candidateSignals.map(s=>s.strategy)});
  }
  return{schemaVersion:2,study:tradeBothSides?'Profit-only simultaneous CE+PE validation':`${underlying} profit-only directional option suite`,period:{startDate,endDate},methodology:{rules:PROFIT_ONLY_RULES,signals,exits,underlying,spotSymbol,contractSelection:`actual dated ${underlying} option nearest 200 points ITM; no premium anchor`,capital:CAPITAL,capitalAllocation:tradeBothSides?'₹30,000 CE + ₹30,000 PE':'₹60,000 signal-directed leg',tradeBothSides,lotSize},sessions,variants:Object.fromEntries(keys.map(k=>[k,{summary:summarizeProfitOnly(variants[k]),...(tradeBothSides?{paired:summarizePairedTrades(variants[k])}:{}),trades:variants[k]}]))};
}

const args=Object.fromEntries(process.argv.slice(2).filter(x=>x.startsWith('--')).map(x=>{const [k,...v]=x.slice(2).split('=');return[k,v.join('=')]}));
if(process.argv[1]?.endsWith('groww-profit-only-directional-backtest.mjs'))backtestProfitOnly({token:process.env.GROWW_ACCESS_TOKEN,startDate:args.start,endDate:args.end,lotSize:args['lot-size']==='auto'||!args['lot-size']?'auto':Number(args['lot-size']),spacingMs:Number(process.env.GROWW_REQUEST_SPACING_MS||1500),signals:args.signals?.split(',')||SIGNALS,exits:args.exits?.split(',')||EXITS,tradeBothSides:args['trade-both-sides']==='true',underlying:args.underlying||'NIFTY',spotSymbol:args['spot-symbol']||'NSE-NIFTY'}).then(r=>{if(args.out)fs.writeFileSync(args.out,`${JSON.stringify(r,null,2)}\n`);process.stdout.write(`${JSON.stringify({period:r.period,sessions:r.sessions.length},null,2)}\n`)}).catch(e=>{console.error(e.stack||e.message);process.exit(1)});
