...
export function validateEntryRelativeResult(document) {
  ...
  for (const variant of ENTRY_RELATIVE_VARIANTS) {
    ...
    for (const row of rows) {
      ...
      const expectedStop = variant.fixedStopPremium ?? row.entry - 20;
      const expectedTarget = variant.kind === 'fixed_levels' ? variant.fixedTargetPremium
        : variant.kind === 'fixed_2r_absolute' ? row.entry + 2 * (row.entry - expectedStop) : row.entry + 40;
      ...
      if (variant.kind === 'fixed_levels'
          && !(row.entry > variant.fixedStopPremium && row.entry < variant.fixedTargetPremium)) {
        errors.push(`${variant.id} ${row.date}: entry is outside fixed ${variant.fixedStopPremium}-${variant.fixedTargetPremium} band`);
      }
      if (variant.entryFloorPremium != null
          && !(row.entry > variant.entryFloorPremium && row.entry < variant.entryCeilingPremium)) {
        errors.push(`${variant.id} ${row.date}: entry is outside ${variant.entryFloorPremium}-${variant.entryCeilingPremium} cohort band`);
      }
