...
function groupStats(trades,keyFn){const m={};for(const t of trades){const k=keyFn(t);(m[k]??=[]).push(t)}return Object.fromEntries(Object.entries(m).sort(([a],[b])=>a.localeCompare(b)).map(([k,v])=>[k,{current:stats(v,'current'),stress0_5:stats(v,'stress0_5'),stress1_0:stats(v,'stress1_0')}]))}
function consistency(trades){const by={};for(const t of trades){const k=t.date.slice(0,7);(by[k]??=[]).push(t)}const months=Object.values(by),profit=months.filter(v=>v.reduce((s,t)=>s+(t.money.current??0),0)>0).length;return{monthsWithTrades:months.length,profitableMonths:profit,profitableMonthPct:months.length?profit/months.length:null}}
function concentration(trades){const v=trades.map(t=>t.money.current).filter(Number.isFinite),abs=v.map(Math.abs).sort((a,b)=>b-a),gp=v.filter(x=>x>0).reduce((a,b)=>a+b,0),gl=Math.abs(v.filter(x=>x<0).reduce((a,b)=>a+b,0)),absTotal=abs.reduce((a,b)=>a+b,0);return{largestTradeAbs:abs[0]??null,largestTradeAbsShare:absTotal?abs[0]/absTotal:null,top5AbsShare:absTotal?abs.slice(0,5).reduce((a,b)=>a+b,0)/absTotal:null,largestWinnerShare:gp?Math.max(0,...v)/gp:null,largestLoserShare:gl?Math.abs(Math.min(0,...v))/gl:null}}
function findCandidates(raws,rows,fast,slow,variant){
  const out=[];
  for(const raw of raws){
    ...
    if(variant.holdBars>0){
      ...
      out.push({index:j,row:rows[j],direction,originIndex:i,originTime:raw.row.timestamp});
      continue;
    }
    ...
    out.push({index:i,row:raw.row,direction,originIndex:i,originTime:raw.row.timestamp});
  }
  return out;
}
