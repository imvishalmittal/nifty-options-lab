...
Each workflow splits the scope into monthly jobs, serializes its API use, applies historically changing NIFTY lot sizes from the selected option's expiry (so transition months are not treated as one regime), uploads raw monthly artifacts, applies a strict integrity gate, and produces one consolidated artifact. Contract selection searches the provider's complete listed ITM catalog, stopping only after the reference premium is bracketed. A month fails consolidation if any observed session has missing underlying/option data, leaves an unbracketed catalog search incomplete, uses a same-candle entry, has incomplete cost scenarios, or overlaps another partition. Exhausting the complete listed catalog is not a search-boundary failure; in that case the closest available quoted contract remains observable.
