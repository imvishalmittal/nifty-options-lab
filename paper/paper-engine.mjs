...
export function processCompletedBar(position, candle, rules = PAPER_RULES) {
  ...
  // Conservative causal convention: only the stop active before this completed
  // one-minute bar may execute inside it. If touched, do not credit the bar's
  // later high/low because intrabar ordering is unobservable from OHLC.
