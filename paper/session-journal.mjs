...
export function compactPaperSession(status, thread) {
  ...
  const strategyVersions = thread === 'V4' ? ['V4', 'V5'] : ['V2', 'V3-5', 'V3-10', 'V6', 'V7', 'V8', 'V9', 'V10-5', 'V10-10', 'V11'];
  const strategyOutcomes = Object.fromEntries(trades.map((row) => {
    const version = row?.trailStepPoints ? `${row?.strategyVersion}-${row.trailStepPoints}` : String(row?.strategyVersion ?? 'UNKNOWN');
