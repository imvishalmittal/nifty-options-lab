...
async function main() {
  ...
  let spotCandles = [];
  for (let attempt = 0; attempt < 8; attempt++) { const currentClock = indiaParts().time; spotCandles = completedCandles(await fetchCandles(token, 'CASH', 'NSE-NIFTY', date, '09:15', currentClock), currentClock); if (candleAt(spotCandles, '09:25')) break; await sleep(30000); }
